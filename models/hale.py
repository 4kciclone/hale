...
class HALE:
    ...
    def begin_task(self):
        ...
        Saves a snapshot of current weights as the projection anchor.
        """
        L = len(self.blocks)
        for l in range(L):
    ...
    def task_boundary(self):
        ...
        1. Projects current weights self.blocks[l].W back towards the W_init anchors
           along the null space tracked by precision matrix P.
        2. Resets precision matrix P for the next task.
        3. Critic is reset. RLS readout weights W_out and its covariance P_rls
           are NOT reset to preserve learned mapping over tasks.
        """
        L = len(self.blocks)
        for l in range(L):
