...
class HALE:
    ...
    def full_train_step(self, x_seq, y):
        ...
        h_outputs = [torch.zeros(d_s, device=self.device)] * (L + 2)
        T = x_seq.shape[0]
        for t in range(T):
            pixel = x_seq[t]
            h_outputs[0] = self.input_proj @ pixel
            for l in range(L):
        ...
        pred_int, loss_val = self.rls_update(h_detached, label)
        e_t = loss_val
        ...
        for l in range(L):
            ...
            s_beta = self.blocks[l].nudge_phase(s0, r_t, s_prev2, label, config['beta'], config['gamma'], config['N_nudge'])
    ...
    def forward_sequence(self, x_seq):
        ...
        for t in range(x_seq.shape[0]):
            h_outputs[0] = self.input_proj @ x_seq[t]
            for l in range(L):
                r_t = self.reservoirs[l].step(h_outputs[l])
